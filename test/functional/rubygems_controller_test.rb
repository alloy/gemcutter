...
class RubygemsControllerTest < ActionController::TestCase
    ...
    context "On GET to show for any gem" do
      ...
      should "renders owner gems overview links" do
        @owners.each do |owner|
          assert_have_selector "a[href='#{user_rubygems_path(owner)}']"
        end
      end
    end
  ...
  context "On GET to index" do
    setup do
      ...
      @another_gem = Factory(:rubygem, :name => "zeta")
    end
    ...
    context "nested under a user resource" do
      setup do
        @user = Factory(:email_confirmed_user)
        @gems.each { |gem| gem.ownerships.create(:user => @user, :approved => true) }
        Factory(:version, :rubygem => @another_gem)
